...
\hline
\rowcolor[rgb]{0.1, 0.3, 0.5} 
\textcolor{white}{\textbf{ID}} & \textcolor{white}{\textbf{Constraint Description}} & \textcolor{white}{\textbf{Status / Implementation}} \\ \hline

\textbf{BR1} & A Team can contain at most 10 Members. & \textbf{Implemented} \newline via \texttt{VARRAY(10)} in \texttt{Member\_VA}. \\ \hline

\textbf{BR2} & Team territorial assignment is constrained by a single Region reference. & \textbf{Implemented} \newline via \texttt{RegionRef} in the \texttt{Team\_t} object and \texttt{SCOPE FOR (RegionRef)}. \\ \hline

\textbf{BR3} & Booking total cost must be a strictly positive value. & \textbf{Implemented} \newline via \texttt{CHECK (TotalCost > 0)} on \texttt{Booking\_TAB}. \\ \hline

\textbf{BR4} & Team operational count must be synchronized when bookings are inserted, deleted, or reassigned. & \textbf{Implemented} \newline via \texttt{TrgSyncTeamOps}, \texttt{ASSIGN\_TEAM\_TO\_BOOKING}, and helper procedures \texttt{IncrementOperationsCount}/\texttt{DecrementOperationsCount}. \\ \hline

\textbf{BR5} & Customer and booking data must remain valid for email, dates, and booking-domain values. & \textbf{Implemented} \newline via table-level \texttt{CHECK} constraints (email format, positive values, booking domains) and \texttt{TrgBookingDates}. \\ \hline

\textbf{BR6} & Team members and event locations must remain consistent with the domain rules. & \textbf{Implemented} \newline via \texttt{TrgTeamMemberDates}, \texttt{TrgTeamMustHaveMembers}, and table-level checks on \texttt{Location\_TAB}. \\ \hline

\textbf{BR7} & Promotional bookings are allowed only on locations with sufficient equipment capacity. & \textbf{Implemented} \newline via \texttt{TrgCheckLocationCapacity} (rejects \texttt{BookingType = 'Promotional'} when \texttt{EquipmentCapacity < 100}). \\ \hline
...
\texttt{Member\_VA} & Bounded varray (max 10) storing members directly inside \texttt{Team\_t}. \\ \hline
\end{tabularx}
\end{center}
...
In this report section, we list only the real \texttt{Trg*} triggers used for synchronization and domain validation.

\subsubsection*{Trigger Catalog (Real \texttt{Trg*} Objects)}
...
\subsubsection*{SQL Code Extracts (Real \texttt{Trg*} Triggers)}
...
The validation suite in \texttt{scripts/07\_triggertests.sql} executes positive and negative checks with automatic identifiers and transactional cleanup. Focusing only on real \texttt{Trg*} triggers, the relevant outcomes are:
\begin{itemize}
...
\section{Implementation: Streamlit Application}
\subsection{Reference Alignment with \texttt{Project Work.pdf}}
The Streamlit implementation follows the same functional perspective described in the reference report section \textit{GUI Screens}, which is structured around three viewpoints: \textbf{Customer}, \textbf{Team}, and \textbf{Manager}. In this project, the same viewpoints are preserved, but mapped to a multi-page Streamlit architecture instead of a single low-code interface.

\subsection{Technical Architecture}
The web application is implemented as a multi-page Streamlit front-end over Oracle 21c XE.
The architecture is composed of the following software layers:
\begin{itemize}
    \item \textbf{UI layer}: Streamlit pages in \texttt{webapp/Home.py} and \texttt{webapp/pages/*.py}.
    \item \textbf{Session and authentication layer}: shared state managed through \texttt{st.session\_state} and login/logout helpers.
    \item \textbf{Data access layer}: Oracle connection pooling and SQL execution through \texttt{db\_utils.py}.
    \item \textbf{Presentation layer}: centralized labels in \texttt{app\_config.py} and custom visual theme in \texttt{ui\_theme.py}.
\end{itemize}

Compared with the deployment logic described in the reference PDF (separated data and application services), this implementation keeps the same conceptual split between data management and user interaction, while focusing on an exam-friendly local setup.

\subsection{Shared Webapp Modules}
The following modules are shared by all pages and provide consistent behavior:
\begin{itemize}
    \item \texttt{app\_config.py}: central titles/subtitles used across the interface.
    \item \texttt{db\_utils.py}: connection parameters from environment variables, user discovery query, connection-pool initialization, logout flow, and session-state bootstrap.
    \item \texttt{ui\_theme.py}: global CSS theme, section headers, and reusable KPI cards.
    \item \texttt{Home.py}: main landing page with project overview, operation frequencies, trigger summary, and navigation guidance.
\end{itemize}

\subsection{Complete Page-by-Page Functional Description}
\subsubsection*{1) Authentication and Access}
\texttt{1\_Login.py} provides:
\begin{itemize}
    \item automatic loading of candidate Oracle users (\texttt{SYSTEM} and \texttt{C\#\#*});
    \item credential-based login with pooled connection activation;
    \item quick health check query (\texttt{SELECT SYSDATE FROM DUAL});
    \item explicit logout with pool shutdown and session cleanup.
\end{itemize}

\subsubsection*{2) Data Exploration}
\texttt{2\_Tables.py} acts as a schema browser for operational and reporting data. It exposes, through expandable panels, both base object tables and derived views:
\begin{itemize}
    \item \textbf{Core tables}: \texttt{Region\_TAB}, \texttt{Office\_TAB}, \texttt{Team\_TAB}, \texttt{Equipment\_TAB}, \texttt{Customer\_TAB}, \texttt{Municipality\_TAB}, \texttt{Location\_TAB}, \texttt{Booking\_TAB}.
    \item \textbf{Reporting views}: \texttt{ViewTeamMembers}, \texttt{ViewCustomerLocations}, \texttt{ViewCompanyOffices}, \texttt{ViewBookingDetails}, \texttt{ViewCustomerActivity}.
\end{itemize}
This page is used for transparency and verification of database state before and after each operation.

\subsubsection*{3) Operation Pages (OP1--OP5)}
\texttt{Op1\_Register\_New\_Customer.py} (Operation 1, 10/day):
\begin{itemize}
    \item generates the next customer code;
    \item captures customer identity and primary address;
    \item optionally creates one or more event locations in the same transaction;
    \item returns a registration summary and an updated customer list.
\end{itemize}

\texttt{Op2\_Record\_New\_Booking.py} (Operation 2, 300/day):
\begin{itemize}
    \item loads available customers, their locations, and active teams;
    \item generates the next booking identifier;
    \item inserts booking metadata (type, date, duration, placement mode, cost);
    \item binds booking to a location reference and handling office reference;
    \item outputs a booking confirmation and recent-bookings table.
\end{itemize}

\texttt{Op3\_Register\_Event\_Location.py} (Operation 3, 50/day):
\begin{itemize}
    \item selects an existing customer;
    \item generates a new location code;
    \item stores full address plus setup/capacity attributes;
    \item shows aggregated counts by customer and a global location list.
\end{itemize}

\texttt{Op4\_View\_Team\_at\_Location.py} (Operation 4, 20/day):
\begin{itemize}
    \item selects an event location and retrieves its bookings;
    \item shows handling office data for each booking;
    \item expands team details and member lists associated with the operational context;
    \item computes local metrics (bookings, total hours, total revenue) for the selected location.
\end{itemize}

\texttt{Op5\_Location\_Activity\_Report.py} (Operation 5, 5/day):
\begin{itemize}
    \item ranks locations by number of bookings (descending);
    \item provides summary statistics and top-performing locations;
    \item supports detail drill-down per location with booking list;
    \item includes CSV export and city-level activity charting.
\end{itemize}

\subsubsection*{4) Manager Analytics Page (OP6)}
\texttt{Op6\_Manager\_Dashboard.py} provides a managerial analytics cockpit with:
\begin{itemize}
    \item global filters (year, office, top-N teams);
    \item KPI cards (total orders, total revenue, average duration, active offices);
    \item team score panel based on \texttt{N\_Total\_Installations};
    \item historical order trend (monthly and yearly views);
    \item distribution widgets by province and placement mode;
    \item office score table with order volume and average cost.
\end{itemize}

\subsection{Functional Mapping to the Reference GUI Views}
The correspondence with the \texttt{Project Work.pdf} GUI viewpoints is the following:
\begin{itemize}
    \item \textbf{Customer view} (reference PDF): implemented through \texttt{1\_Login.py}, \texttt{Op1\_Register\_New\_Customer.py}, \texttt{Op2\_Record\_New\_Booking.py}, and \texttt{Op3\_Register\_Event\_Location.py}.
    \item \textbf{Team view} (reference PDF): implemented through \texttt{Op4\_View\_Team\_at\_Location.py} and \texttt{Op5\_Location\_Activity\_Report.py}, including team/member and location activity analysis.
    \item \textbf{Manager view} (reference PDF): implemented through \texttt{Op6\_Manager\_Dashboard.py} with KPI-driven operational analytics.
\end{itemize}

\subsection{Screenshot Placeholders (Ready for Final Insert)}
The following figures are prepared to host final screenshots. If an image is not available yet, a placeholder box is rendered and compilation still succeeds.

\begin{figure}[h!]
    \centering
    \IfFileExists{images/streamlit_home.png}{%
        \includegraphics[width=0.95\textwidth]{images/streamlit_home.png}
    }{%
        \fbox{\parbox{0.9\textwidth}{\centering Insert screenshot: Home dashboard (Home.py)}}
    }
    \caption{Streamlit home page and operational overview.}
\end{figure}

\begin{figure}[h!]
    \centering
    \IfFileExists{images/streamlit_login.png}{%
        \includegraphics[width=0.95\textwidth]{images/streamlit_login.png}
    }{%
        \fbox{\parbox{0.9\textwidth}{\centering Insert screenshot: Login page (1\_Login.py)}}
    }
    \caption{Authentication page with connection test tools.}
\end{figure}

\begin{figure}[h!]
    \centering
    \IfFileExists{images/streamlit_tables_views.png}{%
        \includegraphics[width=0.95\textwidth]{images/streamlit_tables_views.png}
    }{%
        \fbox{\parbox{0.9\textwidth}{\centering Insert screenshot: Tables and views browser (2\_Tables.py)}}
    }
    \caption{Operational tables and reporting views browser.}
\end{figure}

\begin{figure}[h!]
    \centering
    \IfFileExists{images/streamlit_customer_ops.png}{%
        \includegraphics[width=0.95\textwidth]{images/streamlit_customer_ops.png}
    }{%
        \fbox{\parbox{0.9\textwidth}{\centering Insert screenshot: Customer operations (OP1, OP2, OP3)}}
    }
    \caption{Customer-side workflows: onboarding, booking insertion, and location registration.}
\end{figure}

\begin{figure}[h!]
    \centering
    \IfFileExists{images/streamlit_team_ops.png}{%
        \includegraphics[width=0.95\textwidth]{images/streamlit_team_ops.png}
    }{%
        \fbox{\parbox{0.9\textwidth}{\centering Insert screenshot: Team operations (OP4, OP5)}}
    }
    \caption{Team and location activity analysis workflows.}
\end{figure}

\begin{figure}[h!]
    \centering
    \IfFileExists{images/streamlit_dashboard.png}{%
        \includegraphics[width=0.95\textwidth]{images/streamlit_dashboard.png}
    }{%
        \fbox{\parbox{0.9\textwidth}{\centering Insert screenshot: Manager dashboard (OP6)}}
    }
    \caption{Manager dashboard with KPI, trend, and distribution analytics.}
\end{figure}

\subsection{Known Limitations and Future Improvements}
\begin{itemize}
    \item The booking object does not currently store a direct \texttt{PlacedBy} reference; customer linkage is obtained through \texttt{AtLocation -> OwnerRef}.
    \item Member analytics requiring cross-team historical views may require additional reporting structures beyond the in-row \texttt{Member\_VA} model.
    \item The central-office/team assignment strategy is deterministic but can be extended with workload balancing policies beyond the current least-loaded heuristic.
\end{itemize}

\section{Conclusion}
The final project delivers a coherent Oracle object-relational schema and a working Streamlit front-end. The report now reflects the implemented solution while clearly separating current behavior from planned enhancements.
